...
class TimerConfig:
    ...
    def timer2_period_config(self, mode_1t: bool, uart_mode: bool = False):
        mcu = self.base
        thl = (mcu.T2H.val << 8) + mcu.T2L.val
        lb = self.timer2_freq_calculate(mode_1t, 0, uart_mode)
        hb = self.timer2_freq_calculate(mode_1t, 65535, uart_mode)
        val = self.timer2_freq_calculate(mode_1t, thl)
        ...
        while True:
            ...
            if len(arg) > 0:
                val = float(arg)
            if lb <= val <= hb:
                thl = self.timer2_thl_calculate(mode_1t, val, uart_mode)
                if 0 < thl < 65536:
                    mcu.T2L.set_bits(int(thl) & 0xFF, 0xFF, 0)
                    mcu.T2H.set_bits(int(thl) >> 8, 0xFF, 0)
                    break

    def timer2_freq_calculate(self, mode_1t: bool, thl: int, uart_mode: bool = False):
        """根据定时器2的TH/TL, 计算频率"""
        mcu = self.base
        if uart_mode: # uart不使用TM2PS预分频
            ...
            # SYSCLK / (TM2PS + 1)
            return (mcu.SYSCLK / (65536 - thl)) if mode_1t else (mcu.SYSCLK / (65536 - thl) / 12)

    def timer2_thl_calculate(self, mode_1t: bool, freq, uart_mode: bool = False):
        """根据定时器2的频率, 计算TH/TL"""
        mcu = self.base
        if uart_mode: # uart不使用TM2PS预分频
            return int(65536 - (mcu.SYSCLK / freq / 4)) if mode_1t else int(65536 - (mcu.SYSCLK / freq / 4 / 12))
